...
#clientes_ui <- function(id) {
  ...
  tagList(
    ...
    tags$style(HTML(paste0("
      ...
      /* ----- CARDS (mesma dimensão e espaçamento) ----- */
      #", ns("contador_box"), ",
      #", ns("media_dist_box"), " {
        position: fixed;
        left: 120px;                          /* atualizado via JS para acompanhar a sidebar */
        width: 200px;                         /* largura padronizada */
        min-height: 75px;                     /* altura padronizada */
      ...
      #", ns("contador_icon"), ", #", ns("media_dist_icon"), " { font-size: 24px; color: #6fc836; }
      #", ns("contador_texto"), ", #", ns("media_dist_texto"), " { display: flex; flex-direction: column; }
      #", ns("contador_label"), ", #", ns("media_dist_label"), " { font-size: 13px; font-weight: 500; color: #444; margin-bottom: -2px; }
      #", ns("contador_valor"), ", #", ns("media_dist_valor"), " { font-size: 20px; font-weight: 800; color: #30660c; }

      /* posições verticais individuais para garantir espaçamento */
      #", ns("contador_box"), " { top: 80px; }
      #", ns("media_dist_box"), " { top: 160px; }   /* ~80px abaixo do primeiro */
    "))),
    ...
    # Card 2: Média da distância  (já existente no seu server)
    div(
      id = ns("media_dist_box"),
      icon("route", id = ns("media_dist_icon")),
      div(
        id = ns("media_dist_texto"),
        div("Média da distância", id = ns("media_dist_label")),
        textOutput(ns("media_dist_valor"))
      )
    ),
    ...
    # --- Script: ancora os DOIS cards à largura da sidebar ---
    ...
    ", ns("contador_box"), ns("media_dist_box"))))
  )
}
...
#clientes_server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    dados_filtrados <- reactive({
      ...
      data <- silver
      if (isTruthy(input$filtro_sexo) && input$filtro_sexo != "Todos") data <- data[data$sexo_final_sicredi == input$filtro_sexo, ]
      if (isTruthy(input$filtro_categoria) && input$filtro_categoria != "Todas") data <- data[data$idade_sicredi == input$filtro_categoria, ]
      if (isTruthy(input$filtro_cnpj) && input$filtro_cnpj != "") data <- data[grepl(input$filtro_cnpj, data$cnpj_associado), ]
    ...
    # >>> NOVO: média da distância (km), ignorando NAs <<<
    output$media_dist_valor <- renderText({
      df <- dados_filtrados()
      v  <- if ("distancia_percorrida" %in% names(df)) mean(df$distancia_percorrida, na.rm = TRUE) else NA_real_
      if (is.na(v)) v <- 0
      paste0(format(round(v, 2), big.mark = ".", decimal.mark = ","), " km")
    })
    ...
    # ---------- ATUALIZA CAMADAS (ordem: 90 → 50 → clientes → agências) ----------
    observe({
      df  <- dados_filtrados()
